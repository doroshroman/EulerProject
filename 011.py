...
def max_prod(digits, shift=4):
    ...
    for i in range(len(digits)):
        prod = utils.seq_prod(map(int, digits[i:i+shift]))
        if prod > max_prod:
            max_prod = prod
    return max_prod
...
if __name__ == "__main__":
    ...
    size = 20
    # Keep only digits
    digits = [token for token in table if token.isdigit()]
    # Make pairs
    rows = []
    step = shift =  2
    
    for i in range(0, len(digits), step):
        rows.append(''.join(digits[i:i+shift]))
    
    cols = []
    for i in range(size): 
        for j in range(i, len(rows), size):
            cols.append(rows[j])
